...
tip_util:'Целевое использование видеопамяти (gpu-memory-utilization). Доля VRAM, которую vLLM резервирует под KV-кэш.',
 mdl_title:'Мониторимая модель (выбирается из списка, список читается из лейба model_name в метриках vLLM).',
 mdl_select_tip:'Выбрать модель для мониторинга. Список моделей читается из метрик vLLM.',
 mdl_unavailable:'модель недоступна',
...
tip_util:'Target video-memory utilization (gpu-memory-utilization). The fraction of VRAM vLLM reserves for the KV cache.',
 mdl_title:'Monitored model (chosen from the list, which is read from the model_name label in vLLM metrics).',
 mdl_select_tip:'Select which model to monitor. The model list is read from vLLM metrics.',
 mdl_unavailable:'model unavailable',
